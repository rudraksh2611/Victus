...
def _spoken_time_natural(h24: int, minute: int) -> str:
    """Spoken-style line for English: clock digits + day part, e.g. '2:30 in the afternoon'."""
    h12 = h24 % 12 or 12
    part = _daypart_phrase(h24)
    return f"{h12}:{minute:02d} {part}"
...
def build_news_segments(lang: str, headlines: list[str]) -> list[str]:
    ...
    for i, title in enumerate(headlines):
        if lang == "hi":
            label = labels_hi[i] if i < len(labels_hi) else str(i + 1)
        else:
            label = str(i + 1)
